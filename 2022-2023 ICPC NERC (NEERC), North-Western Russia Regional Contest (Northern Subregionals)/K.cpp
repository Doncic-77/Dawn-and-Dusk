...
struct line
{
    ...
    //}
    bool on_segment_jls(point q) //直线和线段的交点是否在线段上，只判inmid即可。
    {
        point k1 = s ;
        point k2 = s + e ;
        return in_mid(k1 , k2 , q) && sgn(cross(k1 - q , k2 - k1)) == 0 ; // 第一个条件可以换成点积非正。注意使用乘法的精度。
    }
    int direction(point p2) //p0 = s , p1 = s + e 判断p2与向量p0->p1的位置关系
    {
        ...
        else
        {
            ...
            if(on_segment_jls(p2))  return 5 ; //p2在线段p0->p1上
    ...
    bool same_dir(line l2)
    {
        return Parallel_Orthogonal(l2) == 2 && sgn(dot(e , l2.e)) == 1 ;
    }
    ...
    bool can_segment_intersect(line l2) //线段s->s+e与线段l2.s->l2.s+l2.e
    {
        ...
        if(on_segment_jls(l2.s) || on_segment_jls(l2.s + l2.e) 
        || l2.on_segment_jls(s) || l2.on_segment_jls(s + e))  //端点在另一条线段上的情况
        return true ;
    ...
    db segment_segment_dis(line l2) //线段s->s+e与线段l2.s->l2.s+l2.e
    {
        ...
        else
        {
            ...
            if(l2.on_segment_jls(t))  res = min(res , s.dis(t)) ;
            point t2 = l2.projection(s + e) ;
            if(l2.on_segment_jls(t2))  res = min(res , (s + e).dis(t2)) ;
            point t3 = projection(l2.s) ;
            if(on_segment_jls(t3))  res = min(res , l2.s.dis(t3)) ;
            point t4 = projection(l2.s + l2.e) ;
            if(on_segment_jls(t4))  res = min(res , (l2.s + l2.e).dis(t4)) ;
            return res ;
        }
    }
...
int compare_angle(vec v1 , vec v2)
{
    return v1.getP() < v2.getP() || (v1.getP() == v2.getP() && sgn(cross(v1 , v2)) > 0) ;
}
...
struct Polygon
{
    ...
    int contain_point(point p0)  // 不一定是凸的 2 内部 1 边界 0 外部
    {
        ...
        for(int i = 0 ; i < n ; i ++)
        {
            ...
            l.s = u ;
            l.e = v - u ;
            if(l.on_segment_jls(p0))  return 1 ;
...
int main()
{
    ...
    while(T --)
    {
        ...
        for(int i = 0 ; i < n ; i ++)
            for(auto &[u , v] : mp)
            {
                if(p[i].first.equal(u))  continue ;
                if(p[i].second.equal(u))  continue ;
                vec v1 = p[i].first - u ;
                vec v2 = p[i].second - u ;
                line l ;
                l.s = p[i].first ;
                l.e = p[i].second - p[i].first ;
                if(!l.on_segment_jls(u))  continue ;
                point o = u ;
                int cnt = 0 ;
                int cnt2 = 0 ;
                vector<vec> vv2 ;
                ...
                for(auto &z : v)
                {
                    if(p[i].first.equal(z))  continue ;
                    if(p[i].second.equal(z))  continue ;
                    vec v3 = z - u ;
                    if(sgn(cross(v1 , v3)) == 0)  continue ;
                    cnt += 1 ;
                    if(sgn(cross(v1 , v3)) > 0)  
                    {
                        cnt2 += 1 ;
                        vv2.push_back(v3) ;
                    }
                ...
                sort(vv2.begin() , vv2.end() , argcmp) ;
                sort(vv3.begin() , vv3.end() , argcmp) ;
                auto del = [&](vector<vec> v)
                {
                    int m = v.size() ;
                    for(int i = 0 ; i < m ; i ++)
                    {
                        int j = i ;
                        while(j + 1 < m && sgn(cross(v[j] , v[j + 1])) == 0)  j += 1 ;
                        int c = j - i + 1 ;
                        res -= c * (c - 1) / 2 ;
                        i = j ;
                    }
